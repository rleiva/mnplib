"""
Tests for profile-free nescience-guided AutoML search.
"""
...
def test_no_ensembles_in_internal_automl_search():
    clf_families = {
        searcher.family
        for searcher in NescienceClassifier(mlp_search_options=FAST_MLP)._resolve_searchers()
    }
    reg_families = {
        searcher.family
        for searcher in NescienceRegressor(mlp_search_options=FAST_MLP)._resolve_searchers()
    }
...
def test_linear_svm_searchers_are_internal():
...
def test_mlp_search_is_internal_bounded_and_records_candidates():
    X, y = make_classification(
        n_samples=45,
        n_features=3,
        n_informative=2,
...
def test_explicit_artifact_workflow_is_preserved(monkeypatch):
    ...
    assert calls
    assert all(set(call) == {"subset", "predictions", "model_string"} for call in calls)
